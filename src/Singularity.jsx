...
export default function Singularity() {
  ...
  const doShare = async () => {
    ...
    if (navigator.share) {
      try { await navigator.share({ title: "Singularity", text }); setShareState("shared"); setTimeout(() => setShareState("idle"), 3000); return; } catch (e) { if (e.name === "AbortError") return; }
    }
    ...
    <div style={root}>
      ...
      {phase === "ending" && ending && (
        <div style={{ ...card, textAlign: "center", maxWidth: 720, borderColor: ending.tone === "light" ? "rgba(127,218,143,.25)" : ending.tone === "amber" ? "rgba(201,168,76,.25)" : "rgba(232,64,87,.25)" }}>
          ...
          {/* Share preview */}
          <details style={{ textAlign: "left", marginBottom: 28 }}>
            <summary style={{ fontSize: 14, color: "var(--txd)", cursor: "pointer", fontFamily: "var(--fm)", letterSpacing: 1 }}>NÁHLED SDÍLENÍ</summary>
            <pre style={{ marginTop: 12, fontSize: 12, fontFamily: "var(--fm)", color: "var(--txd)", background: "rgba(255,255,255,0.03)", padding: 16, borderRadius: 10, border: "1px solid var(--bdr)", whiteSpace: "pre-wrap", lineHeight: 1.6 }}>
              {shareText(ending, game, history)}
            </pre>
          </details>
